# ==============================================================================
# R/pacta_core.R
# Pure, individually testable PACTA analysis functions extracted from the
# former scripts/pacta_vietnam_scenario.R monolith (the "PACTA decompose").
#
# Each function below corresponds to one of the script's original numbered
# SECTIONs (1-9). Code was moved verbatim: parameters are the top-level
# variables each section originally read, and each function returns exactly
# the objects downstream sections consume. No analysis logic was changed —
# see CLAUDE.md for the byte-identical-MCB-CSV refactor acceptance bar this
# extraction is held to.
#
# scripts/pacta_vietnam_scenario.R sources this file and calls these
# functions in order; it also still `library()`s the analysis stack
# (dplyr, tidyr, ggplot2, r2dii.*, etc.) before sourcing, so these functions
# resolve their free variables (e.g. sector_classifications from
# r2dii.data) via the calling script's environment exactly as the original
# monolith did.
# ==============================================================================

# --- SECTION 1: LOAD VIETNAM DATA -------------------------------------------

#' Load an engagement's PACTA input CSVs.
#'
...
#' Pre-join the loanbook to PACTA sectors via a VSIC/ISIC extension table.
#'
...
#' Fuzzy-match the classified loanbook against ABCD and prioritize.
#'
#' @param loanbook_classified tbl — output of pacta_prejoin_sectors().
#' @param abcd tbl — raw ABCD (from pacta_load_inputs()$abcd).
#' @param output_dir chr — directory to write the match CSVs into.
...
pacta_match_and_prioritize <- function(loanbook_classified, abcd, output_dir) {
  ...
  write_csv(matched_raw, file.path(output_dir, "01_vn_matched_raw.csv"))
  ...
  write_csv(matched, file.path(output_dir, "02_vn_matched_prioritized.csv"))
  cat("\n")

  list(raw = matched_raw, prioritized = matched, abcd_norm = abcd_norm)
}

# --- SECTION 4: COVERAGE ANALYSIS -------------------------------------------

#' Compute PACTA sector coverage and render the coverage pie chart.
#'
#' @param loanbook_classified tbl — output of pacta_prejoin_sectors().
#' @param matched tbl — prioritized matches (pacta_match_and_prioritize()$prioritized).
#' @param output_dir chr — directory to write the coverage pie PNG into.
...
pacta_coverage <- function(loanbook_classified, matched, output_dir, bank_name, bank_short) {
  ...
  ggsave(file.path(output_dir, "03_vn_coverage_pie.png"), p_pie,
...
#' Compute market-share alignment (power + automotive) and render its charts.
#'
#' @param matched tbl — prioritized matches (pacta_match_and_prioritize()$prioritized).
#' @param abcd_norm tbl — ASCII-normalized ABCD (pacta_match_and_prioritize()$abcd_norm).
#' @param scenario tbl — market-share scenario (pacta_load_inputs()$scenario).
#' @param region tbl — region ISO mapping (pacta_load_inputs()$region).
#' @param output_dir chr — directory to write CSVs and PNGs into.
#' @param bank_short chr — short bank label, used in chart subtitles.
...
pacta_market_share <- function(matched, abcd_norm, scenario, region, output_dir, bank_short,
  ...
  write_csv(ms_portfolio, file.path(output_dir, "04_vn_ms_portfolio.csv"))
  write_csv(ms_company,   file.path(output_dir, "04_vn_ms_company.csv"))
  ...
  if (nrow(power_techmix_data) > 0) {
    ...
    ggsave(file.path(output_dir, "05_vn_power_techmix.png"), p_power_techmix,
  ...
  if (nrow(coal_traj_data) > 0) {
    ...
    ggsave(file.path(output_dir, "06_vn_coal_trajectory.png"), p_coal_traj,
           width = 10, height = 6, dpi = 150)
  ...
  if (nrow(renew_traj_data) > 0) {
    ...
    ggsave(file.path(output_dir, "07_vn_renewables_trajectory.png"), p_renew_traj,
           width = 10, height = 6, dpi = 150)
  ...
  if (nrow(auto_techmix_data) > 0) {
    ...
    ggsave(file.path(output_dir, "08_vn_auto_techmix.png"), p_auto_techmix,
           width = 10, height = 6, dpi = 150)
  ...
  if (nrow(ev_traj_data) > 0) {
    ...
    ggsave(file.path(output_dir, "09_vn_ev_trajectory.png"), p_ev_traj,
           width = 10, height = 6, dpi = 150)
  ...
  list(ms_portfolio = ms_portfolio, ms_company = ms_company, target_pdp8 = target_pdp8)
}

# --- SECTION 6: SDA ANALYSIS (Cement + Steel) -------------------------------

#' Compute SDA (sectoral decarbonization) alignment for cement + steel.
#'
#' Manual SDA: bypasses target_sda() API to avoid version-sensitivity issues.
#' Joins matched loans to ABCD emission_factor and computes a
#' production-weighted mean per sector/year.
#'
#' @param matched tbl — prioritized matches (pacta_match_and_prioritize()$prioritized).
#' @param abcd_norm tbl — ASCII-normalized ABCD (pacta_match_and_prioritize()$abcd_norm).
#' @param co2 tbl — CO2 intensity scenario (pacta_load_inputs()$co2).
#' @param output_dir chr — directory to write the SDA CSV and PNGs into.
#' @param bank_short chr — short bank label, used in chart captions.
...
pacta_sda <- function(matched, abcd_norm, co2, output_dir, bank_short) {
  ...
  write_csv(sda_portfolio, file.path(output_dir, "05_vn_sda_portfolio.csv"))
  ...
  if (nrow(cement_sda) > 0) {
    ...
    ggsave(file.path(output_dir, "10_vn_cement_sda.png"), p_cement,
           width = 10, height = 6, dpi = 150)
  ...
  if (nrow(steel_sda) > 0) {
    ...
    ggsave(file.path(output_dir, "11_vn_steel_sda.png"), p_steel,
           width = 10, height = 6, dpi = 150)
  ...
  list(sda_portfolio = sda_portfolio, sda_target_pdp8 = sda_target_pdp8)
}

# --- SECTION 7: ALIGNMENT GAP CALCULATION (PDP8/NDC scenario) --------------

#' Compute market-share and SDA alignment gaps at 2030 vs PDP8/NDC.
#'
#' @param ms_portfolio tbl — pacta_market_share()$ms_portfolio.
#' @param target_pdp8 chr — pacta_market_share()$target_pdp8 metric name.
#' @param sda_portfolio tbl — pacta_sda()$sda_portfolio.
#' @param sda_target_pdp8 chr — pacta_sda()$sda_target_pdp8 metric name.
#' @param loanbook tbl — the RAW loanbook (pacta_load_inputs()$loanbook; used
#'   for the coal stranded-asset chart, not the classified/matched tibbles).
#' @param output_dir chr — directory to write CSVs and PNGs into.
...
pacta_alignment_gaps <- function(ms_portfolio, target_pdp8, sda_portfolio, sda_target_pdp8,
  ...
  write_csv(ms_alignment_2030, file.path(output_dir, "06_vn_ms_alignment_2030.csv"))
  ...
  write_csv(sda_alignment_2030, file.path(output_dir, "06_vn_sda_alignment_2030.csv"))
  ...
  if (nrow(align_plot_data) > 0) {
    ...
    ggsave(file.path(output_dir, "12_vn_alignment_overview.png"), p_align,
...
#' Base64-encode every generated PACTA chart PNG for embedding in the HTML report.
#'
#' @param output_dir chr — directory the PACTA charts were written into.
...
pacta_encode_charts <- function(output_dir) {
  ...
  for (nm in names(chart_files)) {
    fp <- file.path(output_dir, chart_files[[nm]])
    if (file.exists(fp)) {
...
#' Embed a chart as an HTML fragment, or a placeholder if it wasn't generated.
#'
#' @param imgs list — pacta_encode_charts() output.
#' @param key chr — the chart's key in `imgs`.
#' @param caption chr — optional caption shown under the chart.
#' @return chr — an HTML `<div class="chart-container">...</div>` fragment.
chart_html <- function(imgs, key, caption = "") {
  if (!is.null(imgs[[key]])) {
    paste0(
...
#' Assemble and write the self-contained Vietnam PACTA HTML report.
#'
#' @param bank_name chr — full bank name.
#' @param bank_short chr — short bank label.
#' @param loanbook tbl — the RAW loanbook (pacta_load_inputs()$loanbook).
#' @param matched tbl — prioritized matches (pacta_match_and_prioritize()$prioritized).
#' @param ms_alignment_2030 tbl — pacta_alignment_gaps()$ms_alignment_2030.
#' @param sda_alignment_2030 tbl — pacta_alignment_gaps()$sda_alignment_2030.
#' @param imgs list — pacta_encode_charts() output.
...
<div class="section" id="portfolio">
  ...
  <div class="two-charts">
    ', chart_html(imgs, "coverage_pie", "Phân bổ danh mục: PACTA coverage vs ngoài phạm vi"), '
  </div>
...
<div class="section" id="power">
  ...
  ', chart_html(imgs, "power_techmix", sprintf("Cơ cấu công nghệ điện: %s vs PDP8 target (2025-2030)", bank_short)), '
...
<div class="section" id="automotive">
  ...
  <div class="two-charts">
    ', chart_html(imgs, "auto_techmix", sprintf("Cơ cấu công nghệ ô tô: %s vs NDC target", bank_short)), '
    ', chart_html(imgs, "ev_traj", "Lộ trình xe điện EV: VinFast-driven vs NDC target"), '
  </div>
  <div class="callout callout-danger">
...
<div class="section" id="cement">
  ...
  ', chart_html(imgs, "cement_sda", "Xi măng: Lộ trình cường độ phát thải CO₂ (SDA Method)"), '
  <div class="callout callout-warning">
...
<div class="section" id="steel">
  ...
  ', chart_html(imgs, "steel_sda", "Thép: Cường độ phát thải CO₂ theo phương pháp SDA"), '
  <div class="callout callout-danger">
...
<div class="section" id="alignment">
  ...
  ', chart_html(imgs, "align_overview", "Khoảng cách căn chỉnh theo lĩnh vực vs PDP8/NDC tại 2030"), '
...
<div class="section" id="risk">
  ...
  ', chart_html(imgs, "stranded_risk", "Phơi nhiễm than và khai thác theo nhóm mẹ (tỷ VND)"), '
  <table>
